...
class CurrentIssueLoader {
    ...
    renderCurrentIssue() {
        ...
        const currentIssueHTML = `
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden current-issue-card">
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-0">
                    <!-- Journal cover - Left side -->
                    <div class="lg:col-span-1 p-4 sm:p-6 flex justify-center items-center bg-gray-50 border-b lg:border-b-0 lg:border-r border-gray-100">
                    ...
                    <!-- Content - Right side -->
                    <div class="lg:col-span-2 p-5 sm:p-7 flex flex-col justify-between">
                        <div>
                            <div class="flex items-center gap-2 mb-2">
                                <span class="bg-blue-100 text-blue-800 text-xs font-semibold px-2.5 py-0.5 rounded">Latest Publication</span>
                            </div>
                            <h3 class="text-xl sm:text-2xl font-bold academic-title text-gray-900 mb-3">
                                ${issue.title}
                            </h3>
                            
                            <p class="text-gray-700 mb-4 text-sm sm:text-base leading-relaxed text-justify">
                                ${abstract}
                            </p>
                            
                            <div class="mb-5 grid grid-cols-1 sm:grid-cols-3 gap-2 bg-gray-50 p-3 sm:p-4 rounded-lg border border-gray-100 text-xs sm:text-sm">
                                <div class="text-gray-700 flex items-center">
                                    <i class="far fa-calendar-alt text-blue-600 mr-2"></i>
                                    <span><strong>Published:</strong> ${this.formatDate(issue.publishedDate)}</span>
                                </div>
                                <div class="text-gray-700 flex items-center">
                                    <i class="fas fa-book text-blue-600 mr-2"></i>
                                    <span><strong>Volume:</strong> ${issue.volume}, No. ${issue.number}</span>
                                </div>
                                <div class="text-gray-700 flex items-center">
                                    <i class="fas fa-file-alt text-blue-600 mr-2"></i>
                                    <span><strong>Articles:</strong> ${issue.articles ? issue.articles.length : 0}</span>
                                </div>
                            </div>
                        </div>
                        
                        <div class="flex flex-col sm:flex-row gap-3 justify-between items-stretch sm:items-center pt-2">
                            <button 
                                onclick="window.location.href='issues.html?issue=${issue.id}'" 
                                class="bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white px-6 py-2.5 rounded-lg font-medium transition-colors flex items-center justify-center text-sm shadow-sm">
                                <span>View Articles</span>
                                <i class="fas fa-arrow-right ml-2 text-xs"></i>
                            </button>
                            <a href="all-issues.html" class="text-blue-600 hover:text-blue-800 text-xs sm:text-sm font-medium text-center py-1">
                                Browse All Past Issues <i class="fas fa-chevron-right text-xs ml-1"></i>
                            </a>
                        </div>
                    </div>
                </div>
