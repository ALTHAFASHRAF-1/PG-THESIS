...
getDisplayAuthors(article) {
    ...
    updatePageContent() {
        document.title = `${this.currentIssue.title} - DHIU THESIS FORUM`;
    ...
    updateMetaTags() {
        ...
        metaDesc.content = `${this.currentIssue.title} - DHIU THESIS FORUM - Research Scholars Platform - Published ${this.formatDate(this.currentIssue.publishedDate)}`;
...
`;
    ...
    generateCitation(article) {
        ...
        return {
            apa: `${article.author} (${year}). ${article.title}. DHIU THESIS FORUM, ${issue.volume}(${issue.number}), ${article.pages}. ${article.doi ? `https://doi.org/${article.doi}` : ''}`,

            mla: `${article.author}. "${article.title}." DHIU THESIS FORUM, vol. ${issue.volume}, no. ${issue.number}, ${year}, pp. ${article.pages}.`,

            chicago: `${article.author}. "${article.title}." DHIU THESIS FORUM ${issue.volume}, no. ${issue.number} (${year}): ${article.pages}. ${article.doi ? `https://doi.org/${article.doi}.` : ''}`
        };
    }
}
